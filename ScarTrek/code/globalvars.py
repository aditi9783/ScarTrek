...
# start of extractSeq ##########################
def extractSeq( fname, typeflag ): # extract gene or protein seq from respective file handles. typeflag = 0 for ntseq, typefla
g = 1 for protein seq
    fh = open(fname, 'r')
    seqdict = {}
    gposdict = {}
    ...
    for line in fh:
        if ">" in line: # header line
            ...
            if typeflag == 0: # only gene seq file has location info
                ...
                if locstr.startswith("complement"): # info[2] is location string [location=12468..13016] or [location=compleme
nt(13133..13558)]
                    locstr = locstr.lstrip("complement(")
                    locstr = locstr.rstrip(")]")
                    #gname = gname+"_c" # not all complement genes end with a 'c'. Thus add this distinguishing feature to the
 gene name
                elif locstr.startswith("order"): # only one gene has this. [locus_tag=Rv3216] [location=order(3593369..3593437
,3593439..3593852)]
                    # enter this gene manually, encompassing the entire region covered by this gene
                    locstr = "3593369..3593852"
                start, end = locstr.split('..')
                start = start.lstrip('<') # some gene start positions have location mentioned as [location=<2550340..2551326].
 Remove the "<" sign.
                end = end.lstrip('>') # some gene end positions have location mentioned as 817531..>817866. Remove the '>' sig
n.
                istart = int(start)
                iend = int(end)
...
ntseq, gdict, genes = extractSeq( "H37Rv_genes.txt", 0 )
aaseq = extractSeq( "H37Rv_proteins_from_genbank.txt", 1 )
