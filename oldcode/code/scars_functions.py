...
def findIndels( paramlist ):
    ...
    mpileupfh = open(seqdir+"/mapped/aln.sorted.bam.mpileup", 'r')
    outfh = open(seqdir+"/mapped/"+sd+".indels", 'w')
    n_idl = 0
    for line in mpileupfh:
        content = line.split()
        # if the mpileup file is messed up (less than 5 items in line), then print the directory name and move on to the next file
        if len(content) > 3: # positions with no coverage has len(content) = 4, thus correct lines have len > 3
            try:
                int(content[3]) # and correct files have 4th column as the coverage- an interger
            except ValueError: # if the 4th column does not have an int -> raise exception to avoid terminating code, and print custom message
                print "Incorrect mpileup:", seqdir, sd, line
                break
